...
<form action="{{ route('anggaran.update', $anggaran->id) }}" method="POST">
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Edit Informasi Anggaran</h3>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('nama_anggaran') is-invalid @enderror" 
                               id="nama_anggaran" name="nama_anggaran" 
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <input type="month" class="form-control @error('bulan') is-invalid @enderror" 
                               id="bulan" name="bulan" 
                               value="{{ old('bulan', $anggaran->bulan->format('Y-m')) }}" required>
                        @error('bulan')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <select class="form-control @error('status') is-invalid @enderror" id="status" name="status" required>
                            <option value="aktif" {{ old('status', $anggaran->status) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                            <option value="nonaktif" {{ old('status', $anggaran->status) == 'nonaktif' ? 'selected' : '' }}>Non-Aktif</option>
                        </select>
                        @error('status')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
    ...
    @php
        $detailPemasukan = $anggaran->detailAnggaran->where('jenis', 'pemasukan')->keyBy('kategori_id');
        $detailPengeluaran = $anggaran->detailAnggaran->where('jenis', 'pengeluaran')->keyBy('kategori_id');
        $totalPemasukanExisting = $detailPemasukan->where('kategori_id', null)->first();
        $totalPengeluaranExisting = $detailPengeluaran->where('kategori_id', null)->first();
    @endphp
        ...
        <div class="col-md-6">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @php
                        $existing = $detailPemasukan->get($k->id);
                    @endphp
                    <div class="form-group">
                        <label>{{ $k->nama_kategori }}</label>
                        <input type="number" class="form-control" 
                               name="kategori_pemasukan[{{ $k->id }}]" 
                               value="{{ old('kategori_pemasukan.'.$k->id, $existing ? $existing->jumlah : 0) }}" 
                               min="0" step="0.01">
                        <input type="text" class="form-control mt-1" 
                               name="keterangan_pemasukan[{{ $k->id }}]" 
                               placeholder="Keterangan (opsional)" 
                               value="{{ old('keterangan_pemasukan.'.$k->id, $existing ? $existing->keterangan : '') }}">
                    </div>
                    @empty
        ...
        <div class="col-md-6">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @php
                        $existing = $detailPengeluaran->get($k->id);
                    @endphp
                    <div class="form-group">
                        <label>{{ $k->nama_kategori }}</label>
                        <input type="number" class="form-control" 
                               name="kategori_pengeluaran[{{ $k->id }}]" 
                               value="{{ old('kategori_pengeluaran.'.$k->id, $existing ? $existing->jumlah : 0) }}" 
                               min="0" step="0.01">
                        <input type="text" class="form-control mt-1" 
                               name="keterangan_pengeluaran[{{ $k->id }}]" 
                               placeholder="Keterangan (opsional)" 
                               value="{{ old('keterangan_pengeluaran.'.$k->id, $existing ? $existing->keterangan : '') }}">
                    </div>
                    @empty
